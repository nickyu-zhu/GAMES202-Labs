            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    for (int l = 0; l <= SHOrder; l++) {
                        for (int m = -l; m <= l; m++) {
                            auto coefficientIndex = sh::GetIndex(l, m);
                            SHCoeffiecents[coefficientIndex] += Le * sh::EvalSH(l, m, dir.cast<double>().normalized()) * dw;
...
public:
    ...
    PRTIntegrator(const PropertyList &props)
    {
        ...
        else if (type == "interreflection")
        {
            ...
            m_Bounce = props.getInteger("bounce", 10);
        }
        else
        {
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        if (m_Type != Type::Interreflection) {
            for (int i = 0; i < mesh->getVertexCount(); i++)
            {
                ...
                for (int j = 0; j < shCoeff->size(); j++)
                {
                    ...
                    //m_TransportSHCoeffs.col(i).coeffRef(j) = m_TransportSHCoeffs.col(i).coeffRef(j) / 3.14;
                }
            }
        }
        else
        {
            ...
            Eigen::MatrixXf Indirect_TransportSHCoeffs;
            Indirect_TransportSHCoeffs.resize(SHCoeffLength, mesh->getVertexCount());

            for (int i = 0; i < mesh->getVertexCount(); i++) {
                const Point3f& v_1 = mesh->getVertexPositions().col(i);
                const Normal3f& n_1 = mesh->getVertexNormals().col(i);
                auto shCoeff = calIndirecSHcoeffs(&m_TransportSHCoeffs, scene, v_1, n_1, 0);
                for (int j = 0; j < SHCoeffLength; j++) {
                    Indirect_TransportSHCoeffs.col(i).coeffRef(j) = shCoeff[j];
                }
            }

            m_TransportSHCoeffs += Indirect_TransportSHCoeffs;
    ...
    std::vector<double> calIndirecSHcoeffs(const MatrixXf* directSHCoeff, const Scene* scene, const Point3f& v, const Normal3f& n, int bounce) {

        std::vector<double>indirectSHCoeffs(SHCoeffLength);

        if (bounce >= m_Bounce)
            return indirectSHCoeffs;

        auto shFunc_bounce = [&](double phi, double theta) -> double {
            Eigen::Array3d d = sh::ToVector(phi, theta);
            const auto wi = Vector3f(d.x(), d.y(), d.z());
            auto h = wi.dot(n);
            ...
            if (h > 0.0) {
                ...
                if (scene->rayIntersect(ray, intersection)) {
                    ...
                    auto trident = intersection.tri_index;

                    auto normals = intersection.mesh->getVertexNormals();
                    auto normal_x = normals.col(trident.x()).normalized() * bary.x();
                    auto normal_y = normals.col(trident.y()).normalized() * bary.y();
                    auto normal_z = normals.col(trident.z()).normalized() * bary.z();
                    Normal3f interNormal = Normal3f(normal_x + normal_y + normal_z).normalized();
                    auto nextBounce = calIndirecSHcoeffs(directSHCoeff, scene, intersection.p, interNormal, bounce + 1);
                    
                    for (int i = 0; i < SHCoeffLength; i++) {
                        indirect += (directSHCoeff->col(trident.x()).coeffRef(i) * bary.x() + 
                                     directSHCoeff->col(trident.y()).coeffRef(i) * bary.y() + 
                                     directSHCoeff->col(trident.z()).coeffRef(i) * bary.z()) * h;
                    }
                    return indirect /= SHCoeffLength;
                }
                else
                    return 0;
            }
        ...
        auto shCoeff = sh::ProjectFunction(SHOrder, shFunc_bounce, m_SampleCount);
        for (int j = 0; j < shCoeff->size(); j++)
        {
            indirectSHCoeffs[j] = (*shCoeff)[j];
        }
