...
float GeometrySmith(vec3 N, vec3 V, vec3 L, float roughness)
{
    ...
    float ggx2 = GeometrySchlickGGX(max(dot(N,V),0.0001),roughness);
    float ggx1 = GeometrySchlickGGX(max(dot(N,V),0.0001),roughness);
...
vec3 fresnelSchlick(vec3 F0, vec3 V, vec3 H)
{
    ...
    return F0 + (1.0 - F0) * pow((1.0 - max(dot(V,H),0.0001)),5.0);
}
