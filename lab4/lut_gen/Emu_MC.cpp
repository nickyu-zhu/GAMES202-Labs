...
Vec3f IntegrateBRDF(Vec3f V, float roughness, float NdotV) {
    ...
    for (int i = 0; i < sample_count; i++) {
      // TODO: To calculate (fr * ni) / p_o here
            ...
            float G_smith = GeometrySmith(roughness, dot(l, N), dot(V, N));
            float BRDF_term = D_GGX * G_smith;
            float division = 4.0f * dot(N, l) * dot(N,V);
            BRDF_term = BRDF_term / division;

            A_temp = BRDF_term * dot(N,l) / sampleList.PDFs[i];
