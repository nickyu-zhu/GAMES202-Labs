...
Vec3f ImportanceSampleGGX(Vec2f Xi, Vec3f N, float roughness) {
    ...
    float x_inverse = 1.0f - Xi.x;
    float tan_theta = a * std::sqrtf(Xi.x) / std::sqrtf(x_inverse);
    ...
    Vec3f H = SphericalToVector(theta, phi);
    //H = normalize(H);
...
Vec3f IntegrateBRDF(Vec3f V, float roughness) {
    ...
    for (int i = 0; i < sample_count; i++) {
        ...
        float GGX_term = GeometrySmith(roughness, NoV, NoL);
        A_weight = dot(L,H) * GGX_term / (NoV * NoH);
        B_weight = A_weight;

        // Split Sum - Bonus 2    
        A_weight = A_weight * (1.0f - pow((1.0f - VoH), 5.0f)) ;
        A += A_weight;
        B_weight = B_weight * pow((1.0f - VoH), 5.0f);
        B += B_weight;
    }

    

    return {A / sample_count , B / sample_count, 0};
